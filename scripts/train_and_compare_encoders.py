...
class SignalEncoderTrainer:
    ...
    def train_epoch(self, model_type: str) -> Dict[str, float]:
        """训练一个epoch"""
        self.model.train()
        total_loss = 0.0
        num_batches = 0
        ...
        for batch_idx, (signals, labels) in enumerate(self.train_loader):
            ...
            if model_type == 'autoencoder':
                embeddings, reconstructed = self.model(signals)
                loss = self.autoencoder_loss(reconstructed, signals)
            else:
    ...
    def validate(self, model_type: str) -> Dict[str, float]:
        """验证"""
    ...
    def train(self, num_epochs: int, output_dir: str, model_type: str):
        """训练模型"""
        ...
        for epoch in range(num_epochs):
            ...
            # 训练
            train_metrics = self.train_epoch(model_type)
            print(f"Train Loss: {train_metrics['train_loss']:.4f}")
            
            # 验证
            val_metrics = self.validate(model_type)
            if val_metrics:
...
def evaluate_encoder(encoder, test_signals, test_labels, top_k=5):
    """
    评估编码器的检索性能
    """
...
def main():
    parser = argparse.ArgumentParser(description="Train and Compare Signal Encoders")
    parser.add_argument("--output_dir", type=str, default="outputs/encoder_comparison", help="Output directory")
    ...
    encoder_types = [
        ('lstm', SignalEncoder),
        ('autoencoder', AutoencoderSignalEncoder),
        ('cnn', CNNSignalEncoder),
    ...
    for encoder_name, encoder_class in encoder_types:
        print(f"\n{'='*80}")
        ...
        # 训练
        encoder_output_dir = os.path.join(args.output_dir, encoder_name)
        trainer.train(
        ...
        # 加载最佳模型
        best_model_path = os.path.join(encoder_output_dir, "best_model.pt")
        ...
        accuracy = evaluate_encoder(model, val_signals, val_labels)
        print(f"{encoder_name} encoder accuracy: {accuracy:.2%}")
    ...
    for encoder_name, result in results.items():
        print(f"{encoder_name}: Accuracy = {result['accuracy']:.2%}, Best Val Loss = {result['best_val_loss']:.4f}")
    
    # 找出最佳编码器
    best_encoder = max(results, key=lambda x: results[x]['accuracy'])
    print(f"\nBest encoder: {best_encoder} with accuracy {results[best_encoder]['accuracy']:.2%}")
