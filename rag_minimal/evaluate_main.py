...
EVALUATION_QUESTIONS = [
    {
        "query": "电动舵机故障诊断的主要方法有哪些？",
        "relevant_docs": [
            "/mnt/PRO6000_disk/swd/servo_0/refrag/rag_minimal/data/input_docs/基于人工智能的飞机电动舵机故障诊断方法研究_王剑.pdf"
        ],
        "expected_topics": ["FMECA", "FMEMECA", "AFMECA", "多参数联合监测"]
    },
    {
        "query": "FMECA 在该研究中的作用是什么？",
        "relevant_docs": [
            "/mnt/PRO6000_disk/swd/servo_0/refrag/rag_minimal/data/input_docs/基于人工智能的飞机电动舵机故障诊断方法研究_王剑.pdf"
        ],
        "expected_topics": ["故障模式", "危害性分析", "映射关系"]
    },
    {
        "query": "文中提到了哪些关键故障模式？",
        "relevant_docs": [
        ...
        "expected_topics": ["相间短路", "匝间短路", "轴承磨损", "滚道磨损"]
    },
    {
        "query": "故障检测主要依赖哪些参数？",
        "relevant_docs": [
        ...
        "expected_topics": ["电流", "温度", "振动", "转速", "传感器"]
    },
    {
        "query": "该研究的方法与传统方法相比有什么特点？",
        "relevant_docs": [
            "/mnt/PRO6000_disk/swd/servo_0/refrag/rag_minimal/data/input_docs/基于人工智能的飞机电动舵机故障诊断方法研究_王剑.pdf"
        ],
        "expected_topics": ["人工智能", "系统性", "结构化"]
    }
]
...
def run_single_evaluation(query_info: Dict[str, Any], 
    ...
    if ingest_result.returncode != 0:
        result['ingest'] = {'status': 'failed', 'time': ingest_time}
        return result
    
    result['ingest'] = {
        'status': 'success',
        'time': round(ingest_time, 2)
    }
    ...
    if chunk_result.returncode != 0:
        result['chunk'] = {'status': 'failed', 'time': chunk_time}
        return result
    ...
    if retrieve_result.returncode != 0:
        result['retrieve'] = {'status': 'failed', 'time': retrieve_time}
        return result
    ...
    retrieval_result = evaluate_retrieval(
        query=query,
        context_path=str(processed_dir / 'eval_retrieved.json'),
        relevant_docs=relevant_docs,
        k=5
    )
    ...
    report_cmd = [
        ...
        '--query', query,
        '--context', str(processed_dir / 'eval_retrieved.json'),
        '--model', model,
        '--base-url', base_url,
        '--save-run',
...
def generate_summary(results: List[Dict], total_time: float) -> Dict[str, Any]:
    ...
    # 统计指标
    ingest_times = [r.get('ingest', {}).get('time', 0) for r in results]
    chunk_times = [r.get('chunk', {}).get('time', 0) for r in results]
